...
export default function Home() {
  ...
  const getFilteredProducts = (tabId: string) => {
    ...
    return products.filter((p) => p.categoryId === tabId || (p.category && (p.category._id === tabId || p.category.slug === tabId)));
  };

  const filteredProducts = useMemo(() => getFilteredProducts(activeTab), [activeTab, products]);

  // Derived categories for sections
  const snacksCats = homeData.categories?.filter((c: any) => ['snacks', 'cold-drinks', 'biscuits-bakery', 'tea-coffee'].includes(c.slug)) || [];
  const beautyCats = homeData.categories?.filter((c: any) => ['beauty', 'personal-care', 'oral-care', 'baby-care'].includes(c.slug)) || [];
  const householdCats = homeData.categories?.filter((c: any) => ['cleaning', 'kitchenware'].includes(c.slug)) || [];
      ...
      <div ref={contentRef} className="bg-neutral-50 -mt-2 pt-1 space-y-5 md:space-y-8 md:pt-4">
        ...
        {activeTab === 'all' && (
          <>
              ...
              <CategoryTileSection
                ...
                tiles={homeData.bestsellers && homeData.bestsellers.length > 0
                  ? homeData.bestsellers.map((p: any) => ({
                    ...
                    price: p.price,
                    discount: p.discount,
                    productCount: p.variations?.length || 0
                  }))
                  : []
                }
                columns={3}
            ...
            {/* Grocery & Kitchen Section - Show all or filtered subset? Showing top 8 for variety */}
            <CategoryTileSection
