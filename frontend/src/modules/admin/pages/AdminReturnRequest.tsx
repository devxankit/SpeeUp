...
export default function AdminReturnRequest() {
  ...
  const filteredRequests = returnRequests.filter(request =>
    ...
    request.orderItemId.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
        <h1 className="text-2xl font-semibold text-neutral-800">Return Request</h1>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden">
        ...
        <div className="bg-green-500 px-4 sm:px-6 py-3">
          <h2 className="text-white text-lg font-semibold">View Return Request</h2>
        </div>
        ...
        <div className="p-4 sm:p-6 border-b border-neutral-200">
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            ...
            <div className="flex flex-col sm:flex-row gap-3 flex-1 flex-wrap">
              ...
              {/* Filter by Seller */}
              <div className="flex items-center gap-2">
                <label className="text-sm text-neutral-700 whitespace-nowrap">Filter by Seller:</label>
                <select
                ...
                >
                  {sellers.map((seller) => (
                    <option key={seller} value={seller === 'All Seller' ? 'all' : seller}>
                      {seller}
                    </option>
                  ))}
                </select>
              ...
              {/* Filter by Status */}
              <div className="flex items-center gap-2">
                <label className="text-sm text-neutral-700 whitespace-nowrap">Filter by Status:</label>
                <select
          ...
          <table className="w-full min-w-[1400px]">
            ...
            <tbody className="bg-white divide-y divide-neutral-200">
              {displayedRequests.length === 0 ? (
                <tr>
                  <td colSpan={11} className="px-4 sm:px-6 py-8 text-center text-sm text-neutral-500">
                ...
                displayedRequests.map((request) => (
                  <tr key={request.id} className="hover:bg-neutral-50">
                    <td className="px-4 sm:px-6 py-3 text-sm text-neutral-900">{request.orderItemId}</td>
                    <td className="px-4 sm:px-6 py-3 text-sm text-neutral-900 font-medium">{request.user}</td>
                    <td className="px-4 sm:px-6 py-3 text-sm text-neutral-600">{request.product}</td>
                    <td className="px-4 sm:px-6 py-3 text-sm text-neutral-600">{request.variant}</td>
                    <td className="px-4 sm:px-6 py-3 text-sm text-neutral-900">₹{request.price.toFixed(2)}</td>
                    <td className="px-4 sm:px-6 py-3 text-sm text-neutral-900">₹{request.discPrice.toFixed(2)}</td>
                    <td className="px-4 sm:px-6 py-3 text-sm text-neutral-600">{request.quantity}</td>
                    <td className="px-4 sm:px-6 py-3 text-sm text-neutral-900 font-medium">₹{request.total.toFixed(2)}</td>
                    ...
                    <td className="px-4 sm:px-6 py-3 text-sm text-neutral-600">{request.date}</td>
                    <td className="px-4 sm:px-6 py-3">
                      <div className="flex items-center gap-2">
                        <button
