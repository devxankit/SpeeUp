...
export default function AdminShopByStore() {
  ...
  const [stores, setStores] = useState<ShopByStoreTile[]>([...shopByStoreTiles]);
  ...
  const handleAddStore = () => {
    ...
    if (editingId !== null) {
      ...
      setStores(stores.map(store => 
        store.id === editingId 
          ? { 
              ...
              id: storeId.trim(),
              bgColor: bgColor,
              productImages: storeImage ? [URL.createObjectURL(storeImage)] : store.productImages
            } 
          : store
      ));
      ...
      if (stores.some(s => s.id === storeId.trim())) {
        alert('Store ID already exists. Please use a different ID.');
        return;
      }
      ...
      const newStore: ShopByStoreTile = {
        ...
        name: storeName.trim(),
        bgColor: bgColor,
        productImages: storeImage ? [URL.createObjectURL(storeImage)] : ['/assets/shopbystore/default.jpg'],
      };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
        <h1 className="text-2xl font-semibold text-neutral-800">Shop by Store</h1>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden">
          ...
          <div className="p-4 sm:p-6 space-y-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-neutral-500 mt-1">Used for routing (e.g., /store/spiritual)</p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-neutral-200 overflow-hidden">
                ...
                {displayedStores.length === 0 ? (
                  ...
                  displayedStores.map((store) => (
                    <tr key={store.id} className="hover:bg-neutral-50">
                      <td className="px-4 sm:px-6 py-3 text-sm text-neutral-900 font-mono">{store.id}</td>
                      <td className="px-4 sm:px-6 py-3 text-sm text-neutral-900 font-medium">{store.name}</td>
                      <td className="px-4 sm:px-6 py-3">
                        <div className="w-20 h-16 bg-neutral-100 rounded overflow-hidden flex items-center justify-center">
                          {store.productImages && store.productImages.length > 0 ? (
                            <img
          ...
          <div className="px-4 sm:px-6 py-3 border-t border-neutral-200 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0">
            <div className="text-xs sm:text-sm text-neutral-700">
              Showing {startIndex + 1} to {Math.min(endIndex, sortedStores.length)} of {sortedStores.length} entries
            </div>
            <div className="flex items-center gap-2">
              <button
