...
export default function AdminManageRoles() {
  ...
  const [roles, setRoles] = useState<Role[]>([
    { id: 1, role: 'Super Admin', type: 'System' },
    { id: 2, role: 'Admin', type: 'System' },
    { id: 3, role: 'Accountant', type: 'System' },
    { id: 4, role: 'Manager', type: 'System' },
    { id: 5, role: 'Supervisor', type: 'Custom' },
  ]);
  ...
  const handleAddRole = () => {
    ...
    if (editingId !== null) {
      ...
      setRoles((prev) =>
        prev.map((role) =>
          role.id === editingId ? { ...role, role: roleName.trim() } : role
        )
      );
  ...
  const handleDelete = (id: number) => {
    const role = roles.find((r) => r.id === id);
    if (role && role.type === 'Custom') {
      if (window.confirm(`Are you sure you want to delete the role "${role.role}"?`)) {
  ...
  const totalPages = Math.ceil(sortedRoles.length / entriesPerPage);
  const startIndex = (currentPage - 1) * entriesPerPage;
  const endIndex = startIndex + entriesPerPage;
  const displayedRoles = sortedRoles.slice(startIndex, endIndex);
    ...
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="p-6 pb-0">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-semibold text-neutral-800">Manage Roles</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-neutral-200 flex flex-col">
            <div className="bg-white px-6 py-4 border-b border-neutral-200">
              <h2 className="text-lg font-semibold text-neutral-800">View Roles</h2>
            </div>
            <div className="p-6 flex-1 flex flex-col">
              {/* Controls Bar */}
                    ...
                    {displayedRoles.length === 0 ? (
                      ...
                      displayedRoles.map((role) => (
                        <tr key={role.id} className="border-b border-neutral-200 hover:bg-neutral-50">
                          <td className="px-4 py-3 text-sm text-neutral-700">{role.id}</td>
                          <td className="px-4 py-3 text-sm text-neutral-700">{role.role}</td>
              ...
              <div className="mt-4 flex flex-col sm:flex-row justify-between items-center gap-4 pt-4 border-t border-neutral-200">
                <div className="text-sm text-neutral-600">
                  Showing {displayedRoles.length > 0 ? startIndex + 1 : 0} to {Math.min(endIndex, sortedRoles.length)} of {sortedRoles.length} entries
                </div>
                <div className="flex items-center gap-2">
                  <button
