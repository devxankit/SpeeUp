...
export default function AdminProductRequest() {
  ...
  // Fetch product requests on component mount and when filters change
  useEffect(() => {
    ...
    const fetchProductRequests = async () => {
      try {
        ...
        if (response.success) {
          ...
          const requestItems: ProductRequestItem[] = response.data.map(
            (product: Product) => ({
              _id: product._id,
          ...
          setProductRequests(requestItems);
        } else {
  ...
  const handleApprove = async (productId: string) => {
    try {
      ...
      const response = await approveProductRequest(productId, "Active");

      if (response.success) {
        // Update the local state
        setProductRequests((prev) =>
          prev.map((req) =>
            req._id === productId ? { ...req, status: "Active" } : req
          )
        );
        alert("Product approved successfully!");
      } else {
        alert(
          "Failed to approve product: " + (response.message || "Unknown error")
        );
      }
    } catch (err) {
  ...
  const handleReject = async (productId: string) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setProductRequests((prev) =>
          prev.map((req) =>
            req._id === productId ? { ...req, status: "Rejected" } : req
          )
        );
        alert("Product rejected successfully!");
      } else {
        alert(
          "Failed to reject product: " + (response.message || "Unknown error")
        );
      }
    } catch (err) {
  ...
  // Note: Filtering is done server-side, so we just use the productRequests as is
  const displayedRequests = productRequests;
              ...
              {loading ? (
                ...
                displayedRequests.map((request) => (
                  <tr key={request._id} className="hover:bg-neutral-50">
                    ...
                    <td className="px-4 sm:px-6 py-3">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${request.status === "Active"
