...
export default function OrderChart({ title, data, maxValue, height = 400 }: OrderChartProps) {
  ...
  const createSmoothPath = () => {
    ...
    const xAxisY = padding.top + graphHeight;
          ...
          {points.map((point) => {
            ...
            return (
              <circle
                ...
                cy={point.y}
                r={isHovered ? 8 : 5}
                fill={isHovered ? '#9333ea' : '#a855f7'}
                stroke="white"
                strokeWidth={isHovered ? 4 : 3}
                style={{ 
                  transition: 'all 0.2s ease',
                  filter: isHovered ? 'drop-shadow(0 0 8px rgba(147, 51, 234, 0.9))' : 'drop-shadow(0 2px 4px rgba(168, 85, 247, 0.3))'
                }}
              />
            );
          })}
        ...
        {hoveredIndex !== null && tooltipPosition && (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-neutral-700 font-semibold">Orders: <span className="text-purple-600 font-bold">{data[hoveredIndex]?.value}</span></span>
            </div>
          </div>
        )}
