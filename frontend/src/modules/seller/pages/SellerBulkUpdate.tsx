...
export default function SellerBulkUpdate() {
    ...
    const handleDownloadData = () => {
        // Create a sample CSV file with Product IDs for updating
        const sampleCSV = `Product ID,Product Name *,Brand ID *,Category ID *,Subcategory ID,Description,Publish Status *,Popular Status *,Deal Of The Day Status *,Tax ID,Manufacturer,Made In,Is Returnable *,Maximum Return Days,Total Allowed Quantity *,FSSAI NO,Product variant ID,Product variant title,Price *,Discounted Price,Stock *,Status *
1,Sample Product Updated,1,2,3,Updated description,1,0,0,1,Sample Manufacturer,India,1,7,10,12345678909876,1,100Gm,100,80,50,1`;

        const blob = new Blob([sampleCSV], { type: 'text/csv' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'product_bulk_update_data.csv';
                    ...
                    <div className="bg-pink-50 border border-pink-200 px-4 sm:px-6 py-4 space-y-4">
                        ...
                        <div className="bg-yellow-100 border-l-4 border-yellow-500 p-3">
                            <p className="text-sm font-semibold text-yellow-800">
                                Always take a backup of Database mysql before proceeding, so if something went wrong while updating you can roll out always.
                            </p>
                        </div>

                        <div className="space-y-3 text-sm text-neutral-700">
                            <p className="font-semibold">Steps to bulk upload of products:</p>
                        ...
                        <div className="space-y-3 text-sm text-neutral-700">
                            ...
                            <div className="space-y-2 font-mono text-xs">
                                <p><span className="font-semibold">Product ID</span> -&gt; ID of the product to update.</p>
                        ...
                        <div className="space-y-3 text-sm text-neutral-700">
                            ...
                            <div className="space-y-2 font-mono text-xs ml-4">
                                <p><span className="font-semibold">Product variant ID</span> -&gt; ID of the variant to update.</p>
                        ...
                        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-3 mt-4">
                            <p className="text-sm text-yellow-800">
                                <span className="font-semibold">Note</span> - Do not set empty field in variants. if you have no value on specific column then add "0" (zero) in that column.
                            </p>
                        </div>
                    </div>
