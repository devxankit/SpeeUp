...
const TRANSACTIONS: Transaction[] = []; // Empty as shown in image
...
export default function SellerWalletTransaction() {
    ...
    const filteredTransactions = TRANSACTIONS.filter(transaction => {
        ...
        const matchesMethod = methodFilter === 'All' || transaction.flag === methodFilter;
    ...
    return (
        <div className="flex flex-col h-full min-h-screen bg-neutral-50">
            ...
            <div className="bg-teal-600 text-white px-4 sm:px-6 py-3">
                <h1 className="text-lg sm:text-xl font-semibold">View Transaction List</h1>
            </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-neutral-200 flex flex-col">
                    ...
                    <div className="p-4 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-neutral-100">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 flex-1">
                            ...
                            {/* Method Filter */}
                            <div className="flex items-center gap-2">
                                <label className="text-sm text-neutral-600 whitespace-nowrap">Filter by Method:</label>
                                <select
                                    value={methodFilter}
                                ...
                                {displayedTransactions.length === 0 ? (
                                    ...
                                    displayedTransactions.map((transaction) => (
                                        <tr key={transaction.id} className="hover:bg-neutral-50">
                                            <td className="p-4 border border-neutral-200 text-sm text-neutral-900">{transaction.id}</td>
                                            <td className="p-4 border border-neutral-200 text-sm text-neutral-900">{transaction.sellerName}</td>
                                            <td className="p-4 border border-neutral-200 text-sm text-neutral-900">{transaction.orderId}</td>
                                            <td className="p-4 border border-neutral-200 text-sm text-neutral-900">{transaction.orderItemId}</td>
                                            <td className="p-4 border border-neutral-200 text-sm text-neutral-900">{transaction.productName}</td>
                                            <td className="p-4 border border-neutral-200 text-sm text-neutral-900">{transaction.variation}</td>
                                            <td className="p-4 border border-neutral-200 text-sm text-neutral-900">{transaction.flag}</td>
                                            <td className="p-4 border border-neutral-200 text-sm text-neutral-900">₹{transaction.amount.toFixed(2)}</td>
                                            <td className="p-4 border border-neutral-200 text-sm text-neutral-900">{transaction.remark}</td>
                                            <td className="p-4 border border-neutral-200 text-sm text-neutral-900">{transaction.date}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
