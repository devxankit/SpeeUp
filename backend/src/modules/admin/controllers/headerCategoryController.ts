...
export const getAdminHeaderCategories = async (req: Request, res: Response) => {
    try {
...
export const getHeaderCategories = async (_req: Request, res: Response) => {
    try {
        const categories = await HeaderCategory.find({ status: 'Published' }).sort({ order: 1, createdAt: -1 });
        res.json(categories);
    } catch (error) {
        res.status(500).json({ message: 'Server Error', error });
    }
...
export const createHeaderCategory = async (req: Request, res: Response) => {
    try {
        const { name, iconLibrary, iconName, slug, relatedCategory, status, order } = req.body;

        const categoryExists = await HeaderCategory.findOne({ slug });
        if (categoryExists) {
            return res.status(400).json({ message: 'Header category already exists' });
        }
    ...
    try {
        ...
        if (category) {
            ...
            if (slug && slug !== category.slug) {
                ...
                if (slugExists) {
                    return res.status(400).json({ message: 'Theme/Slug already used by another category' });
                }
            }
            ...
            res.status(404).json({ message: 'Header category not found' });
        }
    } catch (error: any) {
        console.error('Update Header Category Error:', error);
        if (error.code === 11000) {
            return res.status(400).json({ message: 'Category with this slug/theme already exists' });
        }
...
export const deleteHeaderCategory = async (req: Request, res: Response) => {
    try {
        ...
        if (category) {
            ...
            res.json({ message: 'Header category removed' });
        } else {
            res.status(404).json({ message: 'Header category not found' });
        }
    } catch (error) {
